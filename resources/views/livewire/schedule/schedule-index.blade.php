...
<div class="py-12">
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6"
                    ...
                    <div class="flex items-center space-x-4 mb-4 md:mb-0">
                        <button class="p-2 rounded-full hover:bg-gray-100" wire:click="prevPeriod">
                        ...
                        <button class="p-2 rounded-full hover:bg-gray-100" wire:click="nextPeriod">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 5l7 7-7 7" />
                        ...
                        <button class="px-3 py-1 bg-gray-100 rounded-md hover:bg-gray-200 text-sm" wire:click="today">
                            今日
                        </button>
                    </div>
                    ...
                    <div class="bg-white rounded-lg border shadow overflow-hidden">
                        ...
                        <div class="grid grid-cols-7 bg-gray-50">
                            @foreach (['日', '月', '火', '水', '木', '金', '土'] as $dayOfWeek)
                                <div
                                    class="py-2 text-center {{ $dayOfWeek === '日' ? 'text-red-600' : ($dayOfWeek === '土' ? 'text-blue-600' : '') }}">
                                    {{ $dayOfWeek }}
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="grid grid-cols-7 border-t">
                            ...
                            @while ($day <= $endOfCalendar)
                                ...
                                <div class="min-h-[120px] border-b border-r p-1 {{ $isCurrentMonth ? '' : 'bg-gray-50' }} {{ $isToday ? 'bg-indigo-50' : '' }}"
                                    wire:click="selectDate('{{ $day->format('Y-m-d') }}')">
                                    <div class="flex justify-between items-center mb-1">
                                        <span
                                            class="text-sm font-medium {{ $day->dayOfWeek === 0 ? 'text-red-600' : ($day->dayOfWeek === 6 ? 'text-blue-600' : '') }} {{ $isToday ? 'bg-indigo-600 text-white rounded-full w-6 h-6 flex items-center justify-center' : '' }}">
                                            {{ $day->day }}
                                        </span>
                                    ...
                                    <div class="space-y-1">
                                        @foreach ($dayEvents as $event)
                                            <div class="text-xs p-1 rounded truncate cursor-pointer {{ $event['status'] === 'completed' ? 'bg-green-100 text-green-800' : 'bg-indigo-100 text-indigo-800' }}"
                    ...
                    <div class="bg-white rounded-lg border shadow overflow-hidden">
                        ...
                        <div class="grid grid-cols-8 bg-gray-50">
                            ...
                            @for ($i = 0; $i < 7; $i++)
                                ...
                                @endphp
                                <div
                                    class="py-2 text-center {{ $day->dayOfWeek === 0 ? 'text-red-600' : ($day->dayOfWeek === 6 ? 'text-blue-600' : '') }}">
                                    <div>{{ ['日', '月', '火', '水', '木', '金', '土'][$day->dayOfWeek] }}</div>
                                    <div
                                        class="{{ $isToday ? 'bg-indigo-600 text-white rounded-full w-6 h-6 flex items-center justify-center mx-auto' : '' }}">
                                        {{ $day->day }}
                                    </div>
                                </div>
                                @php
                        ...
                        <div class="grid grid-cols-8 border-t">
                            @for ($hour = 6; $hour < 22; $hour++)
                                <div
                                    class="h-12 border-b text-xs text-gray-500 flex items-center justify-center border-r">
                                    {{ $hour }}:00
                                </div>
                                ...
                                @for ($i = 0; $i < 7; $i++)
                                    ...
                                    <div class="h-12 border-b border-r {{ $isToday ? 'bg-indigo-50' : '' }}"
                                        wire:click="selectDate('{{ $day->format('Y-m-d') }}')">
                        ...
                        <div class="grid grid-cols-1">
                            @for ($hour = 6; $hour < 22; $hour++)
                                ...
                                <div class="flex border-b group hover:bg-gray-50 cursor-pointer"
                                    ...
                                    <div
                                        class="w-16 flex-shrink-0 text-xs text-gray-500 p-2 border-r flex flex-col justify-center items-center">
                                        <span>{{ sprintf('%02d:00', $hour) }}</span>
                                        <span>-</span>
                                        <span>{{ sprintf('%02d:00', $hour + 1) }}</span>
                                    </div>

                                    <div class="flex-grow p-2">
                                        @foreach ($slotEvents as $event)
                                            <div class="mb-1 p-2 rounded truncate {{ $event['status'] === 'completed' ? 'bg-green-100 text-green-800' : 'bg-indigo-100 text-indigo-800' }}"
                                                wire:click.stop="editEvent({{ $event['id'] }})">
                                        ...
                                        @if ($slotEvents->count() === 0)
                                            <div
                                                class="h-full w-full flex items-center justify-center text-gray-400 text-sm opacity-0 group-hover:opacity-100">
                                                + クリックで予定追加
                                            </div>
                                        @endif
                                    </div>
    ...
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50" x-data
        x-show="$wire.showEventModal" x-cloak x-on:keydown.escape.window="$wire.showEventModal = false">
            ...
            <div class="p-6">
                <form wire:submit.prevent="{{ $eventId ? 'updateEvent' : 'createEvent' }}">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label for="startDateTime"
                                class="block text-sm font-medium text-gray-700 mb-1">開始日時</label>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            <label for="notificationMinutes"
                                class="block text-sm font-medium text-gray-700 mb-1">通知</label>
                        ...
                        @if ($learningMethod)
                            <div class="mt-2 p-3 bg-indigo-50 rounded-md text-sm">
                                @if ($learningMethod === 'pomodoro')
                                    <p><span class="font-medium">ポモドーロテクニック</span>:
                                        25分の集中作業と5分の休憩を繰り返す方法です。4セット終了後に長めの休憩を取ります。</p>
                                ...
                                @elseif($learningMethod === 'intensive')
                                    <p><span class="font-medium">短期集中学習</span>: 短期間に集中して学習する方法です。試験直前の詰め込み学習に適しています。
                                    </p>
                                @elseif($learningMethod === 'output_focused')
                                    <p><span class="font-medium">アウトプット中心学習</span>:
                                        学んだ内容を自分の言葉で説明したり問題を解いたりする方法です。理解度を深めるのに効果的です。</p>
                                @endif
                            </div>
                        @endif
                    </div>
